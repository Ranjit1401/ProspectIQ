...
/** One live progress event emitted by a real backend agent mid-run. */
...
export const workspaceService = {
  ...
  /**
   * Live version of runSupervisor(): opens a Server-Sent Events
   * connection to /executor/stream and reports each agent's real
   * progress as it happens, then resolves with the same shape
   * runSupervisor() returns once the pipeline finishes.
   *
   * `onStep` fires once per backend progress event — the label/agent
   * text is generated by whichever agent is actually running, not by
   * anything hardcoded on the frontend.
   */
  streamSupervisor(
    prompt: string,
    onStep: (step: WorkspaceLiveStep) => void,
  ): Promise<SupervisorResponse> {
    return new Promise((resolve, reject) => {
      let settled = false;

      apiStream(
        `/executor/stream?prompt=${encodeURIComponent(prompt)}`,
        (frame) => {
          try {
            const payload = JSON.parse(frame.data);

            if (frame.event === "step") {
              onStep(payload as WorkspaceLiveStep);
            } else if (frame.event === "final") {
              settled = true;
              resolve(payload as SupervisorResponse);
            } else if (frame.event === "error") {
              settled = true;
              reject(new Error(payload?.message || "The pipeline failed."));
            }
          } catch {
        ...
        },
        {
          ...
          onDone: () => {
            if (!settled) {
              settled = true;
              reject(new Error("The connection closed before the pipeline finished."));
            }
          },
        },
      );
